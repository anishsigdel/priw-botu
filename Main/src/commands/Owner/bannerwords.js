...
const küfürEngel = require('../../../../Database/src/Models/küfürKoruma');
...
module.exports = {
  ...
  async execute(message, args) {
    ...
    if (!action || !['aktif', 'kapalı'].includes(action.toLowerCase())) {
      return message.reply('Lütfen geçerli bir seçenek girin: `aktif` veya `kapalı`.');
    }

    try {
      let filter = await küfürEngel.findOne({ guildId });

      if (!filter) {
        filter = new küfürEngel({
          guildId,
          isActive: action.toLowerCase() === 'aktif', 
        });

        await filter.save();
      } else {
        filter.isActive = action.toLowerCase() === 'aktif';
        await filter.save();
      }
      ...
      const embed = new EmbedBuilder()
        .setDescription(`Küfür engelleme sistemi **${status}** hale getirildi. Ayarın tam-mod aktif hale getirilmesi için lütfen aşağıdaki menüden **Yeniden Başat'ı** seçiniz!`);
