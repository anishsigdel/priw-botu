...
const oxy = require("../../../Database/src/Settings/settings.json");
...
const besModal = new ModalBuilder()
    ...
    .setCustomId('kisi')
    .setPlaceholder(`1234567890196482753`)
...
\`\`\``,ephemeral:true})
 ...
 if(value == "oda-create"){
    ...
    button.guild.channels.create({
        ...
        const logChannel = button.guild.channels.cache.find(channel => channel.name === 'özel-oda-log');
        

        if (logChannel) {
            const embedLog = new EmbedBuilder()
                .setDescription(`
                    ${button.member.user.tag} yeni bir özel oda oluşturdu, Oda: **#${name}**`)
                    .setTimestamp();
