 ...
 */
    ...
    run: async (interaction) => {
        ...
        if(!member)return interaction.reply({content:` **Sunucuda böyle bir kullanıcı yok!**`,ephemeral:true})
        let data = await client.db.get(`özeloda_${interaction.member.id}`)
        if(!data)return interaction.reply({content:` **Bir özel odan yok reis!**`,ephemeral:true})
